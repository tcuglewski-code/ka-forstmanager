...
export default function SaatgutStatistikV2Page() {
  ...
  if (error)
    return <div className="p-6 text-red-500 font-medium">Fehler: {error}</div>
  ...
  // Jahresvergleich nach Gesamtmenge sortiert
  ...
  // Balken-Breite (max 100%, relativ zum Max-Wert)
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold flex items-center gap-2">
            ...
            Saatguternte — Statistik
          </h1>
          <p className="text-sm text-gray-500 mt-0.5">Historische Auswertung 2024 &amp; 2025</p>
        </div>
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              ...
              {
                icon: <Euro className="w-4 h-4 text-orange-500" />,
                label: "Lohnkosten",
                ...
                <div key={s.jahr} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                  ...
                  {s.gruppenFuehrer && (
                    <p className="text-sm text-gray-500">
                      Gruppenführer: <span className="text-gray-700 dark:text-gray-300">{s.gruppenFuehrer}</span>
                    </p>
                  )}
                </div>
              )
            ...
            {ranking.map((r) => (
              <div key={r.name} className={`p-3 ${rowClass(r.rang)}`}>
                <div className="flex items-start justify-between">
                  <div>
                    <span className="font-bold text-gray-400 text-sm mr-2">#{r.rang}</span>
                    <span className="font-medium text-sm">{r.name}</span>
                  </div>
                  <span className="font-bold text-green-700 dark:text-green-400 text-sm">{fmt(r.gesamtKg)} kg</span>
                </div>
                {/* Balken */}
                <div className="mt-2 h-1.5 bg-gray-100 dark:bg-gray-700 rounded-full">
                  <div
                    className="h-full bg-green-500 rounded-full"
                    style={{ width: `${(r.gesamtKg / maxKg) * 100}%` }}
                  />
                </div>
          ...
          <div className="hidden sm:block overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 dark:bg-gray-900/50">
                <tr>
                  <th className="px-3 py-2 text-left text-xs text-gray-500 uppercase w-10">#</th>
                  <th className="px-3 py-2 text-left text-xs text-gray-500 uppercase">Name</th>
                  <th className="px-3 py-2 text-right text-xs text-gray-500 uppercase">Gesamt kg</th>
                  <th className="px-3 py-2 text-right text-xs text-gray-500 uppercase">Anteil</th>
                  <th className="px-3 py-2 text-right text-xs text-gray-500 uppercase">Stunden</th>
                  <th className="px-3 py-2 text-right text-xs text-gray-500 uppercase">kg/h</th>
                  <th className="px-3 py-2 text-right text-xs text-gray-500 uppercase">Kosten</th>
                  <th className="px-3 py-2 text-center text-xs text-gray-500 uppercase">Jahre</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                {ranking.map((r) => (
                  <tr key={r.name} className={`hover:bg-gray-50 dark:hover:bg-gray-700/40 transition-colors ${rowClass(r.rang)}`}>
                    <td className="px-3 py-2 font-bold text-gray-400 text-center">{r.rang}</td>
                    <td className="px-3 py-2">
                      <div>
                        <span className="font-medium">{r.name}</span>
                        {/* Mini-Balken */}
                        <div className="mt-1 h-1 bg-gray-100 dark:bg-gray-700 rounded-full w-28">
                          <div
                            className="h-full bg-green-500 rounded-full"
                            style={{ width: `${(r.gesamtKg / maxKg) * 100}%` }}
                          />
                    ...
                    <td className="px-3 py-2 text-right tabular-nums font-semibold">{fmt(r.gesamtKg)} kg</td>
                    <td className="px-3 py-2 text-right tabular-nums text-gray-500">{fmt(r.anteilProzent)} %</td>
                    <td className="px-3 py-2 text-right tabular-nums text-gray-500">
                      {r.gesamtStunden != null ? `${fmt(r.gesamtStunden)} h` : "–"}
                    </td>
                    <td className="px-3 py-2 text-right tabular-nums">
                      {r.kgProStunde != null ? (
                        <span className={r.kgProStunde > 10 ? "text-green-600 font-medium" : ""}>
                          {fmt(r.kgProStunde)}
                        </span>
                      ) : "–"}
                    </td>
                    <td className="px-3 py-2 text-right tabular-nums text-gray-500">{fmtEur(r.gesamtKosten)}</td>
                    <td className="px-3 py-2">
                      <div className="flex gap-1 justify-center flex-wrap">
                        {r.jahre.map((j) => (
                          <span key={j} className="text-xs bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-1.5 py-0.5 rounded">
                            {j}
                          </span>
                        ))}
                      </div>
          ...
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-gray-50 dark:bg-gray-900/50">
                  <tr>
                    <th className="px-3 py-2 text-left text-xs text-gray-500 uppercase">Baumart</th>
                    <th className="px-3 py-2 text-right text-xs text-gray-500 uppercase">2024 (kg)</th>
                    <th className="px-3 py-2 text-right text-xs text-gray-500 uppercase">2025 (kg)</th>
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 dark:bg-gray-900/50">
                <tr>
                  <th className="px-3 py-2 text-left text-xs text-gray-500 uppercase">Register-Nr.</th>
                  <th className="px-3 py-2 text-left text-xs text-gray-500 uppercase">Forstamt</th>
                  <th className="px-3 py-2 text-left text-xs text-gray-500 uppercase">Baumart</th>
                  <th className="px-3 py-2 text-right text-xs text-gray-500 uppercase">Gesamt kg</th>
                  <th className="px-3 py-2 text-right text-xs text-gray-500 uppercase">Einsätze</th>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                {flaechenPerformance.map((f, idx) => (
                  <tr key={idx} className="hover:bg-gray-50 dark:hover:bg-gray-700/40 transition-colors">
                    ...
                    <td className="px-3 py-2 text-right tabular-nums font-semibold">{fmt(f.gesamtKg)} kg</td>
                    <td className="px-3 py-2 text-right tabular-nums text-gray-500">{f.einsaetze}</td>
                    <td className="px-3 py-2 text-right tabular-nums text-gray-500">{f.saisons}</td>
                  </tr>
                ))}
              </tbody>
            </table>
