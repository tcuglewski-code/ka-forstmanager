    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-[var(--color-on-surface-variant)] text-sm">Erntedaten eingeben und Sammler zuordnen</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-[var(--color-surface-container)] border border-border rounded-xl p-6">
          <h2 className="text-[var(--color-on-surface)] font-semibold mb-4">Fläche</h2>

          <div className="relative">
            <label className="block text-sm text-[var(--color-on-surface-variant)] mb-1">Fläche auswählen *</label>
            {selectedFlaeche ? (
              <div className="flex items-center gap-3 p-3 bg-[var(--color-surface-container-highest)] border border-emerald-700 rounded-lg">
                <div className="flex-1">
                  <span className="text-[var(--color-on-surface)] font-mono">{selectedFlaeche.registerNr}</span>
                  <span className="text-[var(--color-on-surface-variant)] ml-2">·</span>
                  <span className="text-emerald-400 ml-2">{selectedFlaeche.baumart}</span>
                  <span className="text-[var(--color-on-surface-variant)] ml-2">{selectedFlaeche.bundesland}</span>
                </div>
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="text-[var(--color-on-surface-variant)] hover:text-red-400 transition-colors text-sm"
                >
                  Ändern
                </button>
              </div>
              ...
              <div className="relative">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-4 py-2 bg-[var(--color-surface-container-highest)] border border-border rounded-lg text-[var(--color-on-surface)] placeholder-zinc-500 focus:outline-none focus:border-emerald-600"
                  />
                </div>
                {flaecheDropdownOpen && filteredFlaechen.length > 0 && (
                  <div className="absolute z-10 w-full mt-1 bg-[var(--color-surface-container-lowest)] border border-border rounded-lg shadow-xl max-h-60 overflow-y-auto">
                    {filteredFlaechen.map((f) => (
                      <button
                        key={f.id}
                        type="button"
                        onClick={() => selectFlaeche(f)}
                        className="w-full px-4 py-3 text-left hover:bg-surface-container-highest transition-colors border-b border-border last:border-0"
                      >
                        <span className="text-[var(--color-on-surface)] font-mono text-sm">{f.registerNr}</span>
                        <span className="text-emerald-400 ml-3 text-sm">{f.baumart}</span>
                        <span className="text-[var(--color-on-surface-variant)] ml-2 text-xs">{f.bundesland}</span>
                      </button>
                    ))}
                  </div>
                )}
        ...
        {/* Ernte-Details */}
        <div className="bg-[var(--color-surface-container)] border border-border rounded-xl p-6">
          <h2 className="text-[var(--color-on-surface)] font-semibold mb-4">Ernte-Details</h2>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm text-[var(--color-on-surface-variant)] mb-1">Saison (Jahr) *</label>
              <input
                type="number"
                value={saison}
                ...
                max="2100"
                required
                className="w-full px-3 py-2 bg-[var(--color-surface-container-highest)] border border-border rounded-lg text-[var(--color-on-surface)] focus:outline-none focus:border-emerald-600"
              />
            </div>
            <div>
              <label className="block text-sm text-[var(--color-on-surface-variant)] mb-1">Datum *</label>
              <input
                type="date"
                value={datum}
            ...
            <div>
              <label className="block text-sm text-[var(--color-on-surface-variant)] mb-1">Baumart *</label>
              <input
                type="text"
                value={baumart}
            ...
            <div>
              <label className="block text-sm text-[var(--color-on-surface-variant)] mb-1">Gesamtmenge (kg)</label>
              <input
                type="number"
                value={mengeKgGesamt}
            ...
            <div className="col-span-2">
              <label className="block text-sm text-[var(--color-on-surface-variant)] mb-1">Notizen</label>
              <textarea
                value={notizen}
                onChange={(e) => setNotizen(e.target.value)}
                rows={3}
                placeholder="Bemerkungen zur Ernte..."
                className="w-full px-3 py-2 bg-[var(--color-surface-container-highest)] border border-border rounded-lg text-[var(--color-on-surface)] focus:outline-none focus:border-emerald-600 resize-none"
              />
            </div>
          </div>
        </div>
        ...
        <div className="bg-[var(--color-surface-container)] border border-border rounded-xl p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-[var(--color-on-surface)] font-semibold">Sammler / Positionen</h2>
            <button
              type="button"
              onClick={addSammler}
          ...
          {sammler.length === 0 ? (
            <p className="text-[var(--color-on-surface-variant)] text-sm text-center py-6">
              Noch keine Sammler hinzugefügt. Klicke auf „Sammler hinzufügen".
            </p>
          ) : (
            <div className="space-y-3">
              <div className="grid grid-cols-12 gap-2 text-xs text-[var(--color-on-surface-variant)] px-1 mb-1">
              ...
              {sammler.map((s) => (
                <div key={s.id} className="grid grid-cols-12 gap-2 items-center">
                  <input
                    ...
                    placeholder="Name"
                    className="col-span-3 px-3 py-2 bg-[var(--color-surface-container-highest)] border border-border rounded-lg text-[var(--color-on-surface)] text-sm focus:outline-none focus:border-emerald-600"
                  />
                  <input
                    type="date"
                    value={s.datum}
                  ...
                  <input
                    ...
                    step="0.1"
                    min="0"
                    className="col-span-2 px-3 py-2 bg-[var(--color-surface-container-highest)] border border-border rounded-lg text-[var(--color-on-surface)] text-sm focus:outline-none focus:border-emerald-600"
                  />
                  <input
                    type="number"
                    value={s.stunden}
                  ...
                  <button
                    ...
                    className="col-span-2 flex justify-center p-2 text-[var(--color-on-surface-variant)] hover:text-red-400 transition-colors"
                  >
        ...
        <div className="flex items-center justify-end gap-3">
          <Link
            href="/saatguternte/ernte"
            className="px-4 py-2 text-[var(--color-on-surface-variant)] hover:text-white transition-colors"
          >
            Abbrechen
          </Link>
          <button
