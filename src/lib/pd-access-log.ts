...
// Ressourcen-Typen für personenbezogene Daten
export type PdResource =
  | 'USER_PROFILE'      // User-Profil (Name, Email, etc.)
  | 'GPS_DATA'          // GPS-Tracks, Standortdaten
  | 'TIME_ENTRIES'      // Zeiterfassung (Stundeneinträge)
  | 'INVOICE'           // Rechnungen
  | 'MITARBEITER'       // Mitarbeiter-Stammdaten
  | 'DOKUMENT'          // Dokumente (ggf. personenbezogen)
  | 'WALDBESITZER'      // Waldbesitzer-Daten
  | 'KONTAKT'           // Kontakte
  | 'CONSENT'           // Einwilligungen
  | 'AUDIT_LOG'         // Audit-Logs selbst (Meta)
