...
export function ConfirmDialog({ open, title, message, onConfirm, onCancel, danger = true, confirmLabel }: ConfirmDialogProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-[#1a1a1a] border border-border rounded-xl p-6 max-w-sm w-full shadow-2xl">
        ...
        <div className="flex gap-3 mt-6">
          <button onClick={onCancel}
            className="flex-1 px-4 py-2 border border-border rounded-lg text-sm text-zinc-400 hover:text-white hover:border-zinc-500 transition-colors">
            Abbrechen
          </button>
