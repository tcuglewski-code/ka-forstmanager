...
export function ConfirmDialog({ open, title, message, onConfirm, onCancel, danger = true, confirmLabel }: ConfirmDialogProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-[var(--color-surface-container-lowest)] border border-border rounded-xl p-6 max-w-sm w-full shadow-2xl">
        <div className="flex items-start gap-4">
          {danger && <div className="w-10 h-10 rounded-xl bg-red-500/20 flex items-center justify-center flex-shrink-0">
            <AlertTriangle className="w-5 h-5 text-red-400" />
          </div>}
          <div>
            <h3 className="font-semibold text-[var(--color-on-surface)] mb-1">{title}</h3>
            <p className="text-sm text-[var(--color-on-surface-variant)]">{message}</p>
          </div>
        </div>
        <div className="flex gap-3 mt-6">
          <button onClick={onCancel}
            className="flex-1 px-4 py-2 border border-border rounded-lg text-sm text-[var(--color-on-surface-variant)] hover:text-[var(--color-on-surface)] hover:border-zinc-500 transition-colors">
            Abbrechen
          </button>
