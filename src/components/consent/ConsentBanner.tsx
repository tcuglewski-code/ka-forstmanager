...
export function ConsentBanner({ onConsentChange, showOnlyOnce = true }: ConsentBannerProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-end sm:items-center justify-center z-[9999] p-4">
      <div className="bg-[#161616] border border-[#2a2a2a] rounded-xl w-full max-w-lg shadow-2xl">
        {/* Header */}
        ...
        {/* Actions */}
        <div className="flex flex-col sm:flex-row gap-2 p-5 border-t border-[#2a2a2a]">
          <button
            type="button"
            onClick={handleRejectAll}
            disabled={saving}
            className="flex-1 px-4 py-2 rounded-lg border border-[#2a2a2a] text-sm text-zinc-400 hover:text-white hover:border-zinc-500 transition-all disabled:opacity-50"
          >
            Alle ablehnen
          </button>
          <button
